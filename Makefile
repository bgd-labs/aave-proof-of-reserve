...
# Deploy Pool Configurator V2
deploy-v2-pool-configurator-avalanche :;  forge script scripts/DeployV2PoolConfigurator.s.sol:DeployAvax --rpc-url avalanche --broadcast --legacy --ledger --mnemonic-indexes ${MNEMONIC_INDEX} --sender ${LEDGER_SENDER} --verify --etherscan-api-key ${ETHERSCAN_API_KEY} -vvvv

# Deploy Pool Configurator V2
deploy-upgrade-configurator-impl-avalanche :;  forge script scripts/DeployUpgradeConfiguratorImplPayload.s.sol:DeployAvax --rpc-url avalanche --broadcast --legacy --ledger --mnemonic-indexes ${MNEMONIC_INDEX} --sender ${LEDGER_SENDER} --verify --etherscan-api-key ${ETHERSCAN_API_KEY} -vvvv
