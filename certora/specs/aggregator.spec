...
function call_f_with_params(method f, env e, address asset , address PoRFeed, address wrapper,uint16 margin) {
    calldataarg args;
    if (f.selector == sig:enableProofOfReserveFeed(address,address,uint16).selector) {
      enableProofOfReserveFeed(e, asset, PoRFeed, margin);
    } else if (f.selector == sig:disableProofOfReserveFeed(address).selector) {
        disableProofOfReserveFeed(e, asset);
    } else if (f.selector == sig:enableProofOfReserveFeedWithBridgeWrapper(address, address, address, uint16).selector) {
      enableProofOfReserveFeedWithBridgeWrapper(e, asset, PoRFeed, wrapper,margin);
    } else {
        f(e, args);
    }
}
...
rule PoRFeedChange(address asset, address PoRFeed, address wrapper, uint16 margin) {
    
    address feedBefore = getProofOfReserveFeedForAsset(asset);
    address bridgeWrapperBefore = getBridgeWrapperForAsset(asset);
    
    method f; env e;
    call_f_with_params(f, e, asset, PoRFeed, wrapper, margin);

    address feedAfter = getProofOfReserveFeedForAsset(asset);
    address bridgeWrapperAfter = getBridgeWrapperForAsset(asset);

    assert f.selector == sig:enableProofOfReserveFeed(address, address, uint16).selector => (feedAfter != 0 && feedAfter == PoRFeed);
    assert f.selector == sig:enableProofOfReserveFeedWithBridgeWrapper(address, address, address, uint16).selector => 
    ...
    assert (f.selector != sig:enableProofOfReserveFeed(address, address, uint16).selector && 
            f.selector != sig:disableProofOfReserveFeed(address).selector &&
            f.selector != sig:enableProofOfReserveFeedWithBridgeWrapper(address, address, address, uint16).selector) => 
