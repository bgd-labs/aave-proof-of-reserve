...
## How the stETH and eETH Proof of Reserve feed works

For stETH, under the hood, the Chainlink proof of reserves DON every few minutes fetches the balance of all validators in the Consensus Layer, as well as the ETH in the Execution Layer (ETH in the Lido contract plus MEV rewards minus withdrawals), and calculates the total ETH that is covering stETH. Once a day, the Lido Oracle updates the stETH by checking the state of the execution and consensus layer, rebasing the stETH total supply, repaying node validations, and finalizing withdrawal requests.

For eETH, the process is very similar to stETH, however, with an additional step of querying all restaking addresses on EigenLayer. So, it takes a snapshot of the consensus and execution layers and fetches all validators' balances registered on the ether.fi smart contract and calculates the amount of ETH across validators, snapshots of the consensus layer state and the execution layer, and scans all of the validator nodes registered in ether.fi protocol smart contract. Given the snapshots, based on the balances of the withdrawal safe contracts and the validator nodes, it calculates the total amount of ETH staked and accrued for the validator nodes and reports them on-chain.

## Deviations

The two scenarios in which these deviations occur are when there is an update in the Lido Oracle and when the buffered ether is moved (added or removed) from stETH.

For eETH, updates and rebases occur more frequently, every 6 hours, plus the number of restaking addresses, resulting in a higher number of deviations being observed.
...
`Total Supply <= PoR Feed Data`

Below are examples of how the system reacts when the state of LST (e.g., stETH) monitored by the system changes:

1. User deposits 100 ETH, and stETH is minted:

   **New state:** `total supply > por feed data`

   **Reaction**: _If the total supply exceeds the PoR data plus the configured **margin**, the system detects a potential discrepancy and triggers the emergency action._

2. 100 stETH is burned to finalize withdraw requests:

   **New state:** `total supply < PoR feed data`

   **Reaction**: _No emergency action executed. The system interprets this as overcollateralization._

3. The oracle updates, and the asset is rebased:

   **New state possibilities:**

   **A**: `total supply < PoR feed data`

   **B:** `total supply > PoR feed data`

   **C:** `total supply == PoR feed data`

   **R**: _Since after a rebase the total supply can increase or decrease, the system behaves as in the previous two cases, depending on the resulting state._

## Historical data

We collected 3-month data on assets and their proof of reserve data, where we could observe deviations that would trigger a _false-positive_ emergency action; they can be found in the [reports](./reports/) folder. The tables show the `PoR-totalSupply` and its relative percentage to the total supply.

Additionally, we created a [Dune Dashboard](https://dune.com/iflexasp/proof-of-reserves-x-total-supply) that plots the `PoR answer` and `totalSupply` to better visualize the deviations.
