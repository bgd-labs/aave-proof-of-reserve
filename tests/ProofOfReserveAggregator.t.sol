...
contract ProofOfReserveAggregatorTest is PoRBaseTest {
  ...
  function test_enableProofOfReserveFeed(
    ...
    assertEq(
      proofOfReserveAggregator.getReserveProviderForAsset(asset),
      address(0)
    );
  }
  ...
  function test_enableProofOfReserveFeedWithReserveProvider(
    ...
    vm.assume(feed != address(0));
    vm.assume(reserveProvider != address(0));
    _skipAddresses(asset);
    margin = uint16(bound(margin, 0, proofOfReserveAggregator.MAX_MARGIN()));
  ...
  function test_getters() public view {
    ...
    assertEq(
      proofOfReserveAggregator.getReserveProviderForAsset(tokenList.wbtc),
      bridgeWrapper
    );
    assertEq(
